...
export const generatePayrollReport = (payrollData: any, shopName: string = "Your Shop") => {
    const doc = new jsPDF();
    const reportDate = new Date();
    const period = reportDate.toLocaleString('default', { month: 'long', year: 'numeric' });
    ...
    autoTable(doc, {
        startY: 45,
        body: [
            [{ content: 'Total Monthly Payroll', styles: { fontStyle: 'bold' } }, formatCurrency(payrollData.totalMonthlyPayroll)],
            [{ content: 'Active Employees', styles: { fontStyle: 'bold' } }, payrollData.numberOfEmployees],
            [{ content: 'Revenue (Last 30 Days)', styles: { fontStyle: 'bold' } }, formatCurrency(payrollData.totalRevenue)],
            [{ content: 'Revenue Per Employee', styles: { fontStyle: 'bold' } }, formatCurrency(payrollData.revenuePerEmployee)],
        ],
        theme: 'plain',
    });

    // 3. Detailed Employee Table
    const tableBody = payrollData.employees.map((emp: any) => [
        emp.full_name,
        emp.role,
        `${formatCurrency(emp.salary_or_wage)} / ${emp.pay_frequency}`
    ]);

    autoTable(doc, {
        startY: (doc as any).lastAutoTable.finalY + 10,
        head: [['Employee', 'Role', 'Pay Structure']],
        body: tableBody,
        theme: 'grid',
        headStyles: { fillColor: [41, 128, 185] },
    });
    ...
    // 5. Save the PDF
    doc.save(`Payroll_Report_${reportDate.toISOString().split('T')[0]}.pdf`);
}

export const generatePayrollCsv = (employees: any[]) => {
    const reportDate = new Date();
    const headers = ['Employee', 'Role', 'Pay', 'Frequency'];
    ...
    link.setAttribute('href', url);
    link.setAttribute('download', `Payroll_Detail_${reportDate.toISOString().split('T')[0]}.csv`);
    link.style.visibility = 'hidden';
