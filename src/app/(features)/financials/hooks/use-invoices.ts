...
export function useCreateInvoice() {
    ...
    return useMutation({
        mutationFn: async (data: InvoiceFormData & { shop_id: string }) => {
            ...
            const workOrderItems = convertInvoiceItemsToWorkOrderItems(data.invoice_items, data.work_order_id || undefined, data.shop_id)
            const calculations = calculateInvoiceTotals(workOrderItems)
            
            const tax_amount = calculations.subtotal * data.tax_rate
            const total_amount = calculations.subtotal + tax_amount - data.discount_amount
            ...
            const invoiceData = {
                ...
                tax_amount,
                total_amount,
                labor_total: calculations.labourTotal,
...
export function useUpdateInvoice() {
    ...
    return useMutation({
        mutationFn: async ({ id, data }: { id: string; data: Partial<InvoiceFormData> }) => {
            // Fetch existing invoice to get work_order_id and shop_id for calculations
            ...
            if (data.invoice_items) {
                // Use centralized calculation logic
                const workOrderItems = convertInvoiceItemsToWorkOrderItems(
                    data.invoice_items,
                    existingInvoice?.work_order_id || undefined,
                    existingInvoice?.shop_id || undefined
                )
                const calculations = calculateInvoiceTotals(workOrderItems)
                
                const tax_rate = data.tax_rate ?? 0  // Use 0 if null/undefined (tax disabled)
                const tax_amount = calculations.subtotal * tax_rate
                const discount = data.discount_amount || 0
                const total_amount = calculations.subtotal + tax_amount - discount
...
export function useCreateInvoiceFromWorkOrder() {
    ...
    return useMutation({
        mutationFn: async ({ work_order_id, shop_id }: { work_order_id: string; shop_id: string }) => {
            ...
            // Use centralized calculation logic - convert invoiceItems back to WorkOrderItem format
            // Note: items from work_order_items are already WorkOrderItem[], but invoiceItems are transformed
            // We need to use the original items array for calculations
            ...
            const tax_rate = 0.13
            const tax_amount = calculations.subtotal * tax_rate
            const total_amount = calculations.subtotal + tax_amount
...
export function useSyncInvoiceFromWorkOrder() {
    ...
    return useMutation({
        mutationFn: async ({ work_order_id, shop_id }: { work_order_id: string; shop_id: string }) => {
            ...
            const total_amount = calculations.subtotal + tax_amount - discount_amount
