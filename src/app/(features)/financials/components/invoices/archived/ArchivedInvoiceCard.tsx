...
export const ArchivedInvoiceCard: React.FC<ArchivedInvoiceCardProps> = ({ invoice, isSelected = false, onClick }) => {
    ...
    return (
        <Card 
            className={cn(
                "bg-[#131313] border-[#2a2a2a] p-4 transition-all hover:bg-[#1a1a1a] hover:shadow-lg",
        ...
        >
            ...
            <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-1.5">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-400">#{invoice.display_id || invoice.invoice_number}</p>
                    </div>
                </div>
                <div className="flex items-center gap-1">
                    <span className={`${invoice.status === 'paid' ? 'text-green-500' : 'text-red-500'} text-xs px-1.5 py-0.5 rounded-full border ${invoice.status === 'paid' ? 'border-green-800' : 'border-red-800'}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                {/* Customer Info */}
                <div>
                    <p className="text-xs uppercase mb-0.5 text-gray-400">CUSTOMER</p>
                    <p className="text-xs font-medium text-white">{invoice.customer?.customer_name || 'Unknown'}</p>
                    {invoice.customer?.customer_email && (
                        <p className="text-xs text-gray-400">{invoice.customer.customer_email}</p>
                    )}
                </div>
                ...
                {invoice.vehicle && (
                    <div>
                        <p className="text-xs uppercase mb-0.5 text-gray-400">VEHICLE</p>
                        <p className="text-xs font-medium text-white">
                            {invoice.vehicle.year} {invoice.vehicle.make} {invoice.vehicle.model}
                        </p>
                        {invoice.vehicle.license_plate && (
                            <p className="text-xs text-gray-400">Plate: {invoice.vehicle.license_plate}</p>
                        )}
                    </div>
                )}
            ...
            {(invoice.work_order || invoice.payment_method) && (
                <div className="flex items-center justify-between pt-1.5 border-t border-gray-800">
                    <div className="flex items-center gap-1">
                        {invoice.work_order && (
                            <Badge variant="secondary" className="bg-[#2a2a2a] text-gray-300 text-xs px-1 py-0.5">
                                WO: {invoice.work_order.work_order_number}
                            </Badge>
                        )}
                        <Badge variant="secondary" className="bg-gray-700/50 text-gray-400 text-xs px-1 py-0.5">
                            Archived
                        </Badge>
                    </div>
