...
export const InvoiceDetailSheet: React.FC<InvoiceDetailSheetProps> = ({
    ...
    const getStatusBadge = (status: string) => {
        ...
        if (statusLower === 'paid') {
            ...
            return <Badge className="bg-yellow-500/10 text-yellow-400 border-yellow-500/20">Unpaid</Badge>
        } else if (statusLower === 'draft') {
            return <Badge className="bg-gray-500/10 text-gray-400 border-gray-500/20">Draft</Badge>
        } else if (statusLower === 'overdue') {
            return <Badge className="bg-red-500/10 text-red-400 border-red-500/20">Overdue</Badge>
        }
    ...
    const getPriorityBadge = (priority: string) => {
        ...
        return <Badge className="bg-gray-500/10 text-gray-400 border-gray-500/20">Medium</Badge>
    }
    ...
    return (
        <Sheet open={isOpen} onOpenChange={onClose}>
            <SheetContent className="w-[600px] sm:w-[700px] bg-[#0a0a0a] border-[#222222] overflow-y-auto">
                <SheetHeader className="pb-4">
                    ...
                    {invoice.title && (
                        <p className="text-gray-400 text-sm mt-1">{invoice.title}</p>
                    )}
                </SheetHeader>
                ...
                <div className="space-y-6">
                    ...
                    {invoice.customer && (
                        <div className="bg-[#1a1a1a] rounded-lg p-4 border border-[#2a2a2a]">
                            ...
                            <div className="space-y-2 text-sm">
                                <div className="text-white">{invoice.customer.customer_name}</div>
                                {invoice.customer.customer_email && (
                                    <div className="text-gray-400">{invoice.customer.customer_email}</div>
                                )}
                                {invoice.customer.customer_phone && (
                                    <div className="text-gray-400">{invoice.customer.customer_phone}</div>
                                )}
                    ...
                    {invoice.vehicle && (
                        <div className="bg-[#1a1a1a] rounded-lg p-4 border border-[#2a2a2a]">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <h3 className="text-white font-medium">Vehicle</h3>
                            </div>
                    ...
                    {invoice.work_order && (
                        <div className="bg-[#1a1a1a] rounded-lg p-4 border border-[#2a2a2a]">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <h3 className="text-white font-medium">Work Order</h3>
                            </div>
                            <div className="space-y-2 text-sm">
                                <div className="text-white">
                                    {invoice.work_order.work_order_number || invoice.work_order.title}
                                </div>
                                <div className="text-gray-400">Status: {invoice.work_order.status}</div>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="space-y-2 text-sm">
                            ...
                            {invoice.labor_total > 0 && (
                                <div className="flex justify-between">
                                    ...
                                    <span className="text-white">{formatCurrency(invoice.labor_total)}</span>
                                </div>
                            )}
                            {invoice.parts_total > 0 && (
                                <div className="flex justify-between">
                                    <span className="text-gray-400">Parts:</span>
                                    <span className="text-white">{formatCurrency(invoice.parts_total)}</span>
                                </div>
                            )}
                            {invoice.services_total > 0 && (
                                <div className="flex justify-between">
                                    <span className="text-gray-400">Services:</span>
                                    <span className="text-white">{formatCurrency(invoice.services_total)}</span>
                                </div>
                            )}
                            {invoice.fees_total > 0 && (
                                <div className="flex justify-between">
                                    <span className="text-gray-400">Fees:</span>
                                    <span className="text-white">{formatCurrency(invoice.fees_total)}</span>
                                </div>
                            )}
                            ...
                            <div className="flex justify-between">
                                <span className="text-gray-400">Tax ({(invoice.tax_rate * 100).toFixed(1)}%):</span>
                    ...
                    {invoiceItems.length > 0 && (
                        <div className="bg-[#1a1a1a] rounded-lg p-4 border border-[#2a2a2a]">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <h3 className="text-white font-medium">Invoice Items</h3>
                            </div>
                            <div className="space-y-3">
                                {invoiceItems.map((item: any, index: number) => (
                                    <div key={item.id || index} className="bg-[#0f0f0f] rounded p-3 border border-[#2a2a2a]">
                            ...
                            <div className="space-y-2 text-sm">
                                {invoice.payment_method && (
                                    <div className="flex justify-between">
                                        ...
                                        <span className="text-white capitalize">{invoice.payment_method.replace('_', ' ')}</span>
                                    </div>
                                )}
                                {invoice.paid_date && (
                                    <div className="flex justify-between">
                                        <span className="text-gray-400">Paid Date:</span>
                                        <span className="text-white">{formatDate(invoice.paid_date)}</span>
                                    </div>
                                )}
                                {invoice.payment_reference && (
                                    <div className="flex justify-between">
                                        <span className="text-gray-400">Reference:</span>
                                        <span className="text-white">{invoice.payment_reference}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="bg-[#1a1a1a] rounded-lg p-4 border border-[#2a2a2a]">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <h3 className="text-white font-medium">Important Dates</h3>
                        </div>
                        <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="text-white">{formatDateTime(invoice.created_at)}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-400">Updated:</span>
                                <span className="text-white">{formatDateTime(invoice.updated_at)}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    {invoice.description && (
                        <div className="bg-[#1a1a1a] rounded-lg p-4 border border-[#2a2a2a]">
                            ...
                            <p className="text-gray-400 text-sm whitespace-pre-wrap">{invoice.description}</p>
                        </div>
                    )}
                    ...
                    {invoice.migration_metadata && (
                        <div className="bg-[#1a1a1a] rounded-lg p-4 border border-[#2a2a2a]">
                            <h3 className="text-white font-medium mb-2">Migration Data</h3>
                            <div className="text-xs text-gray-400 font-mono bg-[#0f0f0f] p-2 rounded border border-[#2a2a2a] overflow-x-auto">
