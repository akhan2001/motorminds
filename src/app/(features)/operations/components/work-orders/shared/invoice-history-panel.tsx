...
export const InvoiceHistoryPanel: React.FC<InvoiceHistoryPanelProps> = ({
    ...
    const getStatusBadge = (status: string) => {
        ...
        if (statusLower === 'paid') {
            return <Badge className="bg-green-500/10 text-green-400 border-green-500/20 text-xs">Paid</Badge>
        } else if (statusLower === 'unpaid' || statusLower === 'sent') {
            return <Badge className="bg-yellow-500/10 text-yellow-400 border-yellow-500/20 text-xs">Unpaid</Badge>
        } else if (statusLower === 'draft') {
            return <Badge className="bg-gray-500/10 text-gray-400 border-gray-500/20 text-xs">Draft</Badge>
        }
        return <Badge className="bg-blue-500/10 text-blue-400 border-blue-500/20 text-xs">{status}</Badge>
    }
    ...
    return (
        <div className="p-4 space-y-3">
            <div className="flex items-center justify-between mb-4">
                <h4 className="text-sm font-semibold text-white">Invoice History</h4>
                <span className="text-xs text-gray-400">{invoices.length} invoices</span>
            </div>

            <div className="space-y-3 max-h-[calc(100vh-300px)] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
                    {invoices.map((invoice) => (
                        <div
                            key={invoice.invoice_number}
                            className="bg-[#1a1a1a] rounded-lg p-3 border border-[#2a2a2a] hover:border-[#333333] transition-colors cursor-pointer"
                        ...
                        {invoice.paid_date && (
                            <div className="mt-2 pt-2 border-t border-[#2a2a2a] text-xs text-gray-400">
