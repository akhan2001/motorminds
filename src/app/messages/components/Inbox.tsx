...
export default function Inbox({ shopId }: InboxProps) {
    ...
    if (loading) {
        return <p className="text-gray-400 mt-6">Loading conversations...</p>;
    }
    ...
    return (
        <div className="space-y-6 mt-6">
            {pageInfo && !pageInfo.error && (
                <div className="flex items-center gap-4 bg-[#111] p-4 rounded border border-[#222]">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={pageInfo.picture?.data?.url} alt={pageInfo.name} className="w-12 h-12 rounded-full" />
                    <div className="flex-1">
                        <div className="text-lg font-semibold leading-tight">{pageInfo.name}</div>
                        {(() => {
                            const count = (pageInfo.followers_count as number | undefined) ?? (pageInfo.fan_count as number | undefined) ?? null;
                            return count !== null ? (
                                <div className="text-sm text-gray-400">Followers: {count.toLocaleString()}</div>
                            ) : null;
                        })()}
                    </div>
            ...
            {(!pageInfo || pageInfo.error) && (
                <div className="flex items-center justify-between bg-[#111] p-4 rounded border border-[#222]">
                    <p className="text-gray-400 text-sm">Reconnect your Facebook Page to load profile info.</p>
                    <Button
                        variant="outline"
                        size="sm"
                        onClick={() => router.push(`/api/auth/meta/start?shopId=${shopId}`)}
            ...
            {conversations.length === 0 ? (
                ...
                <div className="space-y-4">
                    {conversations.map(conv => {
                        ...
                        return (
                            <div key={conv.id} className="bg-[#111] p-4 rounded border border-[#222] hover:border-red-500 transition-colors">
                                {/* Header */}
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="font-semibold leading-none">{sender?.name ?? "Unknown"}</div>
                                            {lastMsg && (
