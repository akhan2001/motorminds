...
export async function GET(request: NextRequest) {
    try {
        ...
        // Return just the analysis data
        const analysis = finalCallData.call_metadata?.analysis || finalCallData.quote_received || vapiCallData?.analysis
...
export async function POST(request: NextRequest) {
    try {
        ...
        const { parts_request_id, shop_id } = body
        ...
        // Fetch voice call by parts_request_id
        ...
        if (voiceCall.vapi_call_id) {
            try {
                ...
                if (vapiResponse.ok) {
                    vapiCallData = await vapiResponse.json()
                    console.log('Fetched Vapi call data for refresh:', vapiCallData)
                } else {
                    console.error('Vapi API error:', vapiResponse.status, await vapiResponse.text())
                }
            } catch (vapiError) {
        ...
        console.error('Refresh analysis POST API error:', error)
        return NextResponse.json(
            { error: 'Internal server error' },
            { status: 500 }
        )
