...
export async function GET(
	...
	try {
		...
		const resultType = (searchParams.get('resultType') as 'DrillDown' | 'List') || undefined

		const taxonomy = await client.getWiringDiagramsTaxonomyWithRelation(baseVehicleId, contentType, applicationId, {
			engineId,
			resultType
		})
		...
		console.error('[API] Error fetching wiring diagrams taxonomy with relation:', error)
