...
/**
 * GET /api/motor-daas/service-procedures/[baseVehicleId]/details/[applicationId]
...
export async function GET(
	...
	try {
		...
		const result = await client.getServiceProcedureDetails(
			baseVehicleId,
			applicationId,
			engineId ? parseInt(engineId, 10) : undefined
		)

		return NextResponse.json(result)
	} catch (error) {
		console.error('[API] Error fetching service procedure details:', error)
