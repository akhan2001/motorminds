    ...
    useEffect(() => {
        ...
        const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, newSession) => {
            ...
            if (event === 'SIGNED_IN' && newSession) {
                ...
                // Redirect to login in ALL tabs (including other tabs that receive SIGNED_OUT event)
                if (typeof window !== 'undefined' && !window.location.pathname.startsWith('/login')) {
                    console.log('[AuthProvider] Redirecting to login after sign out')
                    // Use window.location.href for hard redirect to ensure all tabs redirect
